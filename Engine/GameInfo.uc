...
function ProcessServerTravel( string URL, bool bItems )
{
    ...
    // CARROT: Modified this so the loading screens get shown before travelling
    Level.NextSwitchCountdown = 0.25;
}
     ...
     foreach DynamicActors(class'Pawn', TestPawn ){
        ...
        (PlayerController(TestPawn.Controller).Player==None) && (TestPawn.Health > 0)
         && (TestPawn.OwnerName~=InName) ){
            ...
            TestPawn.bInitializeAnimation = false; // FIXME - temporary workaround forlack of meshinstance serialization
...
*/
    ...
    NewPlayer.UID = ParseOption(Options, "UID");
...
event PostLogin( PlayerController NewPlayer )
{
    ...
    if (NewPlayer.PlayerReplicationInfo != none) {
        ...
        NewPlayer.PlayerReplicationInfo.xnaddr = NewPlayer.xnaddr;
        NewPlayer.PlayerReplicationInfo.Gamertag = NewPlayer.Gamertag;
        NewPlayer.PlayerReplicationInfo.UID = NewPlayer.UID;
    }
}
