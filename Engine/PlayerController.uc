...
var string BestActionMessage; //prof J.
...
var config string ClientLostConnectionMenuMPXbox;
var config string ClientLostConnectionMenuMP;

var config string ClientConnectionFailedMenuMPXbox;
var config string ClientConnectionFailedMenuMP;
...
replication
{
    ...
    reliable if( bNetDirty && bNetOwner && Role==ROLE_Authority )
        GameReplicationInfo, BestActionMessage;
 ...
 * Called if connection is unexpectedly terminted.
 *****************************************************************
 */

event ClientLostConnection(){

    if (Player.GUIController.bIsConsole) {
        if (!Player.GUIController.CheckExists('XDOTZMPDisconnect')) {
            Player.GUIController.OpenMenu(ClientLostConnectionMenuMPXbox);
        }
    } else {
        if (!Player.GUIController.CheckExists('XDOTZMPDisconnect')) {
            Player.GUIController.OpenMenu(ClientLostConnectionMenuMP);
        }
    }
}
...
exec function SetSensitivityTemp(float F)
{
    PlayerInput.UpdateSensitivityTemp(F);
}
...
exec event SetProgressMessage( int Index, string S, color C )
{
    ...
    if (S != "") {
        if (Player.GUIController.bIsConsole) {
            Log("CARROT: SetProgressMessage " $ S);
            Player.GUIController.CloseTo('XDOTZLoadingMenu');
            Player.GUIController.OpenMenu(ClientConnectionFailedMenuMPXbox,S);
        } else {
            Log("CARROT: SetProgressMessage " $ S);
            Player.GUIController.CloseTo('DOTZLoadingMenu');
            Player.GUIController.OpenMenu(ClientConnectionFailedMenuMP,S);
        }
    }
}
...
defaultproperties
{
     ...
     ClientLostConnectionMenuMPXbox="DOTZMenu.XDOTZMPDisconnect"
     ClientLostConnectionMenuMP="DOTZMenu.DOTZMPDisconnect"
     ClientConnectionFailedMenuMPXbox="DOTZMenu.XDOTZMPFailed"
     ClientConnectionFailedMenuMP="DOTZMenu.DOTZMPFailed"
