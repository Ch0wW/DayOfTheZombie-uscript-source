...
function Report_TargetDestroyed( VGSPAIController c, Actor target ) {
   ...
   if ( bAllTargetsDestroyed ) {
      ...
      if ( target != none ) {
         ...
         //NOTE not quite right... if !bAllTargetsDestroyed && t == None,
         //NOTE something has gone wrong...
         c.StageOrder_HoldPosition();
      }
   }
}
...
function leaveStage( VGSPAIController c, EReason r ) {
   ...
   //NOTE if the controller abandoned an assault position, allocate
   //NOTE someone else to it...
}
