...
function OnTakingDamage(Pawn other, float damage) {
    ...
    if ( !bReturnedFire ) {
        SetMultiTimer( START_FIRE_TIMER, 0.7, false );
        bReturnedFire = true;
    }
    Tactic_Attack();
}
...
BEGIN:
    //NOTE make sure the bot isn't shooting...
...
BEGIN:
    //NOTE now the bot can open fire!
    bot.SetCombatTimer();
    while (true) {
        bot.Perform_Engaged_StandGround();    WaitForNotification();
    }
    //NOTE should probably do something clever once the enemy is dead,
    //NOTE like attack another enemy or hide again...
}
...
function PreBeginPlay() {
    ...
    ForEach AllActors( class'Pawn', p ) {
        if ( p.AIScriptTag == pawnAIScriptTag ) {
            ...
            p.bDontPossess = true;
            // start out looking in the direction of the pawn's facing...
            c.FocalPoint = p.Location 
                + ( (Vect(256,0,0) + (Vect(0,0,1) * p.CollisionHeight))
                      >> p.Rotation );
            return;
        }
    } 
...
defaultproperties
{
     ControllerType=Class'DEBAIT.VGSPAIController'
     hAmbush="AMBUSH"
     bDebugLogging=True
     bHasHandlers=True
}
