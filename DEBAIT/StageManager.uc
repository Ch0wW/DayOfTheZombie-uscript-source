...
function Timer() {
   ...
   if ( playerPawn == None ) {
      ...
      for ( i = 0; i < theStages.length; ++i ) {
         playerDist = VSize( theStages[i].location - playerPawn.Location );
         hibernateDist = theStages[i].ActiveRange + HIBERNATE_FUDGE;
         if ( playerDist > hibernateDist ) theStages[i].hibernate();
         else if ( playerDist < hibernateDist ) theStages[i].awaken();
      }
      // schedule the next check for the amount of time it would take the
      // player to cross the HIBERNATE_FUDGE zone at full speed.
      minTimeToContact 
         = HIBERNATE_FUDGE / playerPawn.GroundSpeed;
   }
